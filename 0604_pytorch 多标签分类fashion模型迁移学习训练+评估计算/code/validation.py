...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str, default="models/2021-07-21_05-06/checkpoint-0150.pth", help="Path to the checkpoint")
    ...
    logdir = os.path.join('logs-val', datetime.strftime(datetime.now(), '%Y-%m-%d_%H-%M'))
    os.makedirs(logdir, exist_ok=True)
    logger = SummaryWriter(logdir)
    epoch = int((os.path.basename(args.checkpoint).split(".")[0]).split("-")[-1]) # checkpoint-000050
    # Visualization of the trained model
    # visualize_grid(model, val_dataloader, attributes, device, checkpoint=args.checkpoint)
    validate(model, val_dataloader, logger, epoch, device, checkpoint=args.checkpoint)