...
class Swish(nn.Module):
    ...
    def forward(self, x):
        return (x * nn.functional.sigmoid(x)) # Swish:   Mish: x*(torch.tanh(nn.functional.softplus(x)))
...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.act = Swish() if act is True else (act if isinstance(act, nn.Module) else nn.Identity()) # elesun nn.SiLU()
    # Sigmoid Tanh ReLU LeakyReLU PReLU ReLU6 SELU GELU SiLU RReLU CELU Hardsigmoid Hardtanh Hardswish Swish? Mish?
    # Swish() Mish() elesun
    def forward(self, x):
