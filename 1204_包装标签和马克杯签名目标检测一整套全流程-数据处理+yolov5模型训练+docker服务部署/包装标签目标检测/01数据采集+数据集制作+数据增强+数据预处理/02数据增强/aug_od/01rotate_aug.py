#!/usr/bin/env python
# https://blog.csdn.net/uncle_ll/article/details/83930861
import cv2
import math
import numpy as np
import os
        ...
        for angle in angle_list:
            for img_name in os.listdir(imgs_path):
                ...
                if s == ".jpg":
                    ...
                    save_name = n + "_angle" + str(angle) + ".jpg"
                    # 写入图像
                    ...
                    for ob in root.iter('object'):
                        ...
                        for box in ob.iter('bndbox'):
                            ...
                            # box.set('updated', 'yes')
                    # write into new xml
                    save_name = n + "_angle" + str(angle) + ".xml"
                    tree.write(os.path.join(xml_save_path, save_name))
                print("[%s] %s is processed." % (angle, img_name))


if __name__ == '__main__':
    img_aug = ImgAugemention()
    imgs_path = 'raw_VOClabel/JPEGImages'
    xmls_path = 'raw_VOClabel/Annotations'
    img_save_path = 'rotateaug_VOClabel/JPEGImages'
    xml_save_path = 'rotateaug_VOClabel/Annotations'
